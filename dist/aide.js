...
export function Aide({ texte, className = '' }) {
    ...
    const largeurCarte = (fenetre) => Math.min(288, fenetre - 16);
    const basculer = () => {
    ...
    return (_jsxs("span", { className: `relative inline-block align-middle ${className}`, children: [_jsx("button", { ref: bouton, type: "button", "aria-label": "Explication", "aria-expanded": ouvert, "aria-controls": id, title: texte, onClick: basculer, onBlur: () => setOuvert(false), className: `inline-flex h-4 w-4 items-center justify-center rounded-full border text-[10px] font-bold leading-none ${ouvert ? 'border-charbon text-charbon' : 'border-charbon/30 text-charbon/55 hover:border-charbon hover:text-charbon'}`, children: "?" }), ouvert && pos && (_jsx("span", { id: id, role: "tooltip", style: { top: pos.top, left: pos.left }, className: "fixed z-50 w-[min(18rem,calc(100vw-1rem))] rounded-lg border-2 border-sable bg-creme px-3 py-2 text-left text-xs font-normal normal-case tracking-normal text-charbon shadow-lg", children: texte }))] }));
}
