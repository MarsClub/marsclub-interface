 ...
 * le `<a>` par défaut.
 */
export function BoutonFermer({ href, className = '', Lien = 'a', }) {
    return (_jsx(Lien, { href: href, className: `${CLASSE_FERMER} ${className}`.trim(), children: "Fermer" }));
}
/**
 * Retirer quelque chose — une note, un filtre. La croix reste discrète et
 * porte toujours un intitulé au survol.
 */
export function CroixRetirer({ titre }) {
    return (_jsx("button", { title: titre, className: "opacity-40 transition-opacity hover:opacity-100", children: "\u00D7" }));
}
